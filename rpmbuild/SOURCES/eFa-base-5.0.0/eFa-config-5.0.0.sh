...
EOF
chmod +x /usr/sbin/eFaFirstBoot.sh

# Use a systemd unit for CentOS 9 (rc.local executes too early)
...
[Service]
ExecStart=/usr/sbin/eFaFirstBoot.sh
...
sed -i "/^Socket / c\Socket inet:8893@127.0.0.1" /etc/opendmarc.conf
